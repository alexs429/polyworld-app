    ...
    btn.addEventListener("click", async (ev) => {
      ...
      try {
        const res = await fetch(
          ENDPOINTS.finalizeEmberTraining,
          {
            method: "POST",
...
export async function resumeEmberTraining(ember) {
  ...
  if (ember.trainingProgress?.complete) {
    ...
    addMsg("assistant", `✅ Your Ember "${ember.name}" is fully trained!`);
    return;
  }
  ...
  if (ember.media?.avatarUrl && (ember.trainingProgress?.step || 0) >= 3) {
    ...
    setActiveEmberUI(ember.id, {
      id: ember.id,
      name: ember.name || ember.identity?.firstName || "Training Ember",
      img: avatarUrl,
      room: roomUrl,
      _raw: ember,
    });
  ...
  window.dispatchEvent(
    new CustomEvent("pw:resume-training-step", { detail: { step, ember } })
  );
}
